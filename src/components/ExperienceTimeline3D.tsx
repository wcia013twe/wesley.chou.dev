...
// ── Feature flags ─────────────────────────────────────────────────────────────
const SHOW_WARP_INTRO = false; // set true to enable the hyperspeed warp intro animation
...
function buildCard(entry: any, section: 'professional' | 'academic', side: 'left' | 'right'): { el: HTMLElement; close: () => void } {
  const CARD_W      = 390;
  ...
  const B_HOVER = isPro ? 'rgba(110, 145, 255, 0.62)' : 'rgba(55, 200, 210, 0.62)';
  const B_OPEN  = isPro ? 'rgba(120, 155, 255, 0.85)' : 'rgba(65, 215, 225, 0.85)';
  const S_IDLE  = isPro
  ...
  const chipBorder = isPro ? 'rgba(80, 108, 215, 0.38)' : 'rgba(40, 175, 185, 0.38)';
  const chipBg     = isPro ? 'rgba(22, 32, 90, 0.55)'   : 'rgba(12, 72, 80, 0.55)';
  const chipColor  = isPro ? 'rgba(168, 185, 255, 0.88)' : 'rgba(100, 225, 235, 0.88)';
  ...
  if (entry.skills?.length) {
    ...
    wrap2.style.cssText = 'display:flex;flex-wrap:wrap;gap:5px;margin-bottom:4px;';
  ...
  if (entry.links && Object.keys(entry.links).length) {
    ...
    linksWrap.style.cssText = 'display:flex;flex-wrap:wrap;gap:8px;margin-bottom:20px;';
  ...
  const open = () => {
    ...
    arrow.style.color      = isPro ? 'rgba(148, 178, 255, 0.90)' : 'rgba(80, 220, 230, 0.90)';
  };
  const close = () => {
    expanded = false;
    ...
    arrow.style.color      = isPro ? 'rgba(110, 140, 228, 0.52)' : 'rgba(50, 185, 195, 0.52)';
  };
...
export default function ExperienceTimeline3D() {
  ...
  useEffect(() => {
    ...
    const bloomPass = new UnrealBloomPass(new THREE.Vector2(w, h), 0.45, 0.4, 0.42);
    ...
    const nodeMat = new THREE.MeshBasicMaterial({ color: 0xdddddd, toneMapped: false });

    // Preload hero images now so they're cached when cards are built later
    allEntries.forEach(entry => { if (entry.hero) { new Image().src = entry.hero; } });
    ...
    allEntries.forEach((entry, i) => {
      ...
      const conn = new THREE.Line(
        ...
        new THREE.LineBasicMaterial({ color: 0x999999, transparent: true, opacity: 0.4, toneMapped: false })
      );
    ...
    const proRingMat = new THREE.MeshBasicMaterial({ color: 0x33ee77, toneMapped: false });
    ...
    const mkProLine = () => {
      ...
      d.style.cssText = 'flex:1;height:1px;background:linear-gradient(to right,transparent,rgba(100,136,255,0.45),transparent);';
      return d;
    };
    ...
    const ringMat = new THREE.MeshBasicMaterial({ color: 0x33ee77, toneMapped: false });
    ...
    const mkLine = () => {
      const d = document.createElement('div');
      d.style.cssText = 'flex:1;height:1px;background:linear-gradient(to right,transparent,rgba(50,215,225,0.45),transparent);';
      return d;
    };
    ...
    allEntries.forEach((entry, i) => {
      ...
      const dotColor = isPro ? 'rgba(248, 113, 113, 0.55)' : 'rgba(252, 165, 165, 0.45)';
      ...
      dot.addEventListener('click', () => { if (introPhase !== 'idle') return; targetZ = entryZ[i]; });
    ...
    const PHASE_DUR = { warp: WARP_DUR, resolve: RESOLVE_DUR, landing: LANDING_DUR };
    ...
    const warpLines = new THREE.LineSegments(beamGeo, new THREE.LineBasicMaterial({
      vertexColors: true,
      transparent: true,
    ...
    const boostTex = new THREE.TextureLoader().load('/textures/energy-beam-opacity.png');
    const engineBoost = new THREE.Mesh(
      new THREE.CylinderGeometry(BOOST_RADIUS_BOT, BOOST_RADIUS_TOP, BOOST_LENGTH, 12),
      ...
      ship.traverse((child) => {
        ...
        mats.forEach((m) => {
          if (m instanceof THREE.MeshStandardMaterial) {
            m.roughness = Math.max(0.20, m.roughness - 0.10);
            m.metalness = Math.min(1.00, m.metalness + 0.15);
          }
        });
      });
    ...
    const nearestStopIdx = () => snapStops.reduce(
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      dismissHint();
      targetZ = Math.max(MIN_Z, Math.min(RING_PRO_STOP, targetZ - e.deltaY * 0.032));
      if (e.deltaY > 0) {
    ...
    function animate(t = 0) {
      ...
      if (introPhase === 'warp' || introPhase === 'resolve') {
        if (delta > 0) {
          ...
          const warpFactor = introPhase === 'warp' ? easeInOutCubic(introProg) : 1.0;
          const fadeAlpha  = introPhase === 'resolve' ? (1 - easeOutCubic(introProg)) : 1.0;
          ...
          for (let i = 0; i < BEAM_N; i++) {
            ...
            beamCol[i*6+0] = 0.2 * bright; beamCol[i*6+1] = 0.35 * bright; beamCol[i*6+2] = bright;
            beamCol[i*6+3] = 0.7 * bright; beamCol[i*6+4] = 0.85 * bright; beamCol[i*6+5] = bright;
          }
          ...
          const starSpeed = introPhase === 'warp'
          ...
          if (introProg >= 1) {
            if (introPhase === 'warp') {
              introPhase = 'resolve';
              introProg  = 0;
            } else { // resolve done — reveal scene, go straight to idle
              warpLines.visible = false;
              proRing.visible   = true;
              ring.visible      = true;
              spine.visible     = true;
              timelineNodes.forEach(n => { n.visible = true; });
              timelineConns.forEach(c => { c.visible = true; });
              stars.visible   = true;
      ...
      if (introPhase === 'landing') {
        ...
        engineBoost.position.set(shipX + BOOST_X_OFFSET, SHIP_Y + BOOST_Y_OFFSET, shipZ + BOOST_Z_OFFSET);
      ...
      // Chase camera
      camera.position.set(mouseX * 0.4, 3.5 + mouseY * 0.25, currentZ + CAM_Z_OFFSET);
      camera.lookAt(0, -0.5, currentZ + SHIP_Z_OFFSET - 2);

      // Ship
      if (ship) {
        const sz = currentZ + SHIP_Z_OFFSET;
        ship.position.set(SHIP_X + mouseX * SHIP_MOUSE_X_AMP, SHIP_Y + Math.sin(time * SHIP_BOB_SPEED) * SHIP_BOB_AMP, sz);
        ...
        shipWarmLight.position.set(ship.position.x + 0.3, ship.position.y - 1.0, sz + 2.5);
        shipCoolLight.position.set(ship.position.x - 0.8, ship.position.y + 1.2, sz + 2.8);
      }
      ...
      if (atProRing) {
        ...
        activeIdx = Math.max(0, Math.min(allEntries.length - 1, Math.round(-currentZ / CARD_SPACING)));
